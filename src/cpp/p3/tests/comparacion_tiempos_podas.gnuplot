...
plot 'tiempos_sin_podas.txt' using 1:(log($2) / (log($1) + $1*log($1) - $1)) with lines linewidth 2 title "log(tiempo) / (log(x) + x*log(x) - x)"
...
plot 'tiempos_con_poda_agregar_camion.txt' using 1:(log($2) / (log($1) + $1*log($1) - $1)) with lines linewidth 2 title "log(tiempo) / (log(x) + x*log(x) - x)"
...
plot 'tiempos_con_ambas_podas.txt' using 1:(log($2) / (log($1) + $1*log($1) - $1)) with lines linewidth 2 title "log(tiempo) / (log(x) + x*log(x) - x)"
